...
class AX12a:
    ...
    def set_initial_state(self, dxl_ids, moving_speeds, initial_positions):
        ...
        self.group_position_read=GroupSyncRead(self.portHandler,self.packetHandler,ADDR_AX_PRESENT_POSITION_L,2)
    ...
    def set_register1_ID(self, reg_num, reg_value,index):
        dxl_comm_result, dxl_error = AX12a.packetHandler.write1ByteTxRx(
            AX12a.portHandler, index, reg_num, reg_value)
        AX12a.check_error(dxl_comm_result, dxl_error)
    ...
    def set_register2_ID(self, reg_num, reg_value,index):
        dxl_comm_result, dxl_error = AX12a.packetHandler.write2ByteTxRx(
            AX12a.portHandler, index, reg_num, reg_value)
        AX12a.check_error(dxl_comm_result, dxl_error)
    ...
    def get_register1_ID(self, reg_num,index):
        reg_data, dxl_comm_result, dxl_error = AX12a.packetHandler.read1ByteTxRx(
            AX12a.portHandler, index, reg_num)
        AX12a.check_error(dxl_comm_result, dxl_error)
        return reg_data
    ...
    def enable_torque_ID(self,index):
        """Enable torque for motor."""
        self.set_register1_ID(ADDR_AX_TORQUE_ENABLE, TORQUE_ENABLE,index)
        print(self.get_register1_ID(ADDR_AX_TORQUE_ENABLE,index))
        # print("Torque has been successfully enabled for dxl ID: %d" % self.id)
    ...
    def disable_torque_ID(self,index):
        """Disable torque."""
        self.set_register1_ID(ADDR_AX_TORQUE_ENABLE, TORQUE_DISABLE,index)
        print(self.get_register1_ID(ADDR_AX_TORQUE_ENABLE,index))
        # print("Torque has been successfully disabled for dxl ID: %d" % self.id)
    ...
    def set_position_ID(self, dxl_goal_position,index):
        """Write goal position."""
        self.set_register2_ID(ADDR_AX_GOAL_POSITION_L, dxl_goal_position,index)
        print("Position of dxl ID: %d set to %d " %
              (index, dxl_goal_position))
    ...
    def initial_position_group(self, positions):
        ...
        self.set_position_group_simple(positions)
    ...
    def set_position_group_simple(self, positions):
        self.update_present_positions()
        for i in range(len(self.DXL_IDS)):
            dxl_id = self.DXL_IDS[i]
            position = int(positions[i])
    ...
    def position_correction(self):
        self.update_present_positions()
        delta_positions = [int(self.PREV_GOAL_POSITIONS[IDX]) - self.PRESENT_POSITIONS[IDX] for IDX in range(len(self.DXL_IDS))]
        for i in range(len(self.DXL_IDS)):
            dxl_id = self.DXL_IDS[i]
            position = int(delta_positions[i])
            self.group_position_write.changeParam(dxl_id, [DXL_LOBYTE(position), DXL_HIBYTE(position)])
    ...
    def set_moving_speed_ID(self, dxl_goal_speed,index):
        """Set the moving speed to goal position [0-1023]."""
        self.set_register2_ID(ADDR_AX_GOAL_SPEED_L, dxl_goal_speed,index)
        print("Moving speed of dxl ID: %d set to %d " %
              (index, dxl_goal_speed))
    ...
    def get_position(self):
        ...
        dxl_present_position:int = self.get_register2(ADDR_AX_PRESENT_POSITION_L)
        print("ID:%03d  PresPos:%03d" % (self.id, dxl_present_position))
        return dxl_present_position

    def get_position_ID(self, index):
        """Read present position."""
        dxl_present_position:int = self.get_register2_ID(ADDR_AX_PRESENT_POSITION_L, index)
        print("ID:%03d  PresPos:%03d" % (index, dxl_present_position))
        return dxl_present_position

    def get_present_speed(self):
        """Returns the current speed of the motor."""
        present_speed:int = self.get_register2(ADDR_AX_PRESENT_SPEED_L)
        print("ID:%03d  PresPos:%03d" % (self.id, present_speed))
        return present_speed

    def get_moving_speed(self):
        """Returns moving speed to goal position [0-1023]."""
        moving_speed:int = self.get_register2(ADDR_AX_GOAL_SPEED_L)
        print("ID:%03d  PresPos:%03d" % (self.id, moving_speed))
        return moving_speed
